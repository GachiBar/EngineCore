...
{
    auto find_result = KeyMapVirtualToEnum.find(KeyCode);
    if(find_result == KeyMapCharToEnum.end())
    {
        find_result = KeyMapCharToEnum.find(CharCode);
    }
    ...
    if(find_result == KeyMapCharToEnum.end() && CharCode > 32)
    {
	...
	return find_result != KeyMapCharToEnum.end() ? find_result->second : EKeys::Invalid;
}
