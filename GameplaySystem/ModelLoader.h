...
public:
	...
	static bool LoadObj(const std::string& path, OpaqueMesh& model) {
		Assimp::Importer importer;
		const aiScene* scene = importer.ReadFile(
...
private:
	...
	static void ProcessNode(OpaqueMesh& model, const aiScene* scene, const aiNode* node) {
		for (size_t i = 0; i < node->mNumMeshes; i++)
		{
			const aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
			ProcessMesh(model, scene, mesh);
	...
	static void ProcessMesh(OpaqueMesh& model, const aiScene* scene, const aiMesh* mesh)
	{
		using namespace DirectX::SimpleMath;

		for (size_t i = 0; i < mesh->mNumVertices; ++i) {
			OpaqueModelVertex vertex;
			vertex.position = ToVector3(mesh->mVertices[i]);
