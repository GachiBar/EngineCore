...
namespace GameplayCore.Mathematics
{
    ...
    public struct Quaternion : IEquatable<Quaternion>, IFormattable
    {
        ...
        public static void ToEuler(Quaternion rotation, out float yaw, out float pitch, out float roll)
        {
            yaw = (float)Math.Atan2(2.0f * (rotation.Y * rotation.W + rotation.X * rotation.Z), 1.0f - 2.0f * (rotation.X * rotation.X + rotation.Y * rotation.Y));
            pitch = (float)Math.Asin(2.0f * (rotation.X * rotation.W - rotation.Y * rotation.Z));
            roll = (float)Math.Atan2(2.0f * (rotation.X * rotation.Y + rotation.Z * rotation.W), 1.0f - 2.0f * (rotation.X * rotation.X + rotation.Z * rotation.Z));
        }
